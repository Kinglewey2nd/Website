...
const CardEditor: React.FC = () => {
  ...
  const handleDelete = async (card: Card) => {
    if (!window.confirm(`Delete card "${card.cardName}"? This cannot be undone.`))
      return;
  ...
  const saveEdit = async (card: Card) => {
    ...
    try {
      await updateDoc(doc(db, "cards", card.id), { cardName: editingName.trim() });
      setCards((prev) =>
        prev.map((c) => (c.id === card.id ? { ...c, cardName: editingName.trim() } : c))
      );
      setEditingId(null);
      setEditingName("");
    } catch (error) {
          ...
          >
            ...
            {editingId === card.id ? (
              <>
                ...
                />
                <br />
                <button onClick={() => saveEdit(card)} style={{ marginTop: "0.5rem" }}>
                  Save
                </button>
                <button
